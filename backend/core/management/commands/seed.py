...
class Command(BaseCommand):
    ...
    def create_characters(self, data):
        for obj in data:
            try:
                ...
                self.stdout.write(f'Character {character.name} already exists, skipping creation...')

            except ObjectDoesNotExist:
                obj['first_appearance'] = date(int(obj['first_appearance']), 1, 1)
                character = Character.objects.create(**obj)
                self.stdout.write(f'Creating Character Object: {character.name}')

    def create_tag_groups(self, data):
        for obj in data:
            tag_group, created = TagGroup.objects.get_or_create(name=obj['name'])
            if not created:
    ...
    def create_artists(self, data):
        ...
        for obj in data:
            ...
            if not created:
                self.stdout.write(f'Artist {artist} already exists, skipping creation...')
            else:
